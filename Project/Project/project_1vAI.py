...
def main():
    ...
    while True:
        ...
        move_paddle_AI(canvas, paddle_2, ball)
...
def end_screen(canvas, score_1, score_2):
    ...
    if score_1 > score_2:
        ...
        text = canvas.create_text(CANVAS_WIDTH/2,CANVAS_HEIGHT/2, "smutko počítač tě porazil")
        canvas.set_fill_color(text, "white")
        canvas.set_font(text, "Arial", 60)
...
def move_paddle_AI(canvas, paddle, ball,):
    x = canvas.get_left_x(paddle)
    if canvas.get_top_y(paddle) <= 0:
        canvas.move_to(paddle, x, 1)
    elif canvas.get_top_y(paddle) >= CANVAS_HEIGHT - PADDLE_HEIGHT:
        ...
        y_ball = canvas.get_top_y(ball)
        y_paddle = canvas.get_top_y(paddle)
        if y_ball > y_paddle:
            canvas.move(paddle, 0, paddle_speed_AI)
        elif y_ball < y_paddle:
            canvas.move(paddle, 0, -paddle_speed_AI)
        else:
            pass
